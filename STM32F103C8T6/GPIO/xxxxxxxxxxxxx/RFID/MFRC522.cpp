...
MFRC522::MFRC522(	uint8_t chipSelectPin,		///< Arduino pin connected to MFRC522's SPI slave select input (Pin 24, NSS, active low)
	...
	//CS_GPIO, CS_PIN = chipSelectPin;
	pinMode(chipSelectPin, OUTPUT);
	digitalWrite(chipSelectPin, HIGH);
	
	// Set the resetPowerDownPin as digital output, do not reset or power down.
	//RESET_GPIO, RESET_PIN = resetPowerDownPin;
	pinMode(resetPowerDownPin, OUTPUT);
	digitalWrite(resetPowerDownPin, LOW);
...
void MFRC522::PCD_WriteRegister(	byte reg,		///< The register to write to. One of the PCD_Register enums.
	...
	SPI.transfer(value);
	digitalWrite(CS_PIN, HIGH);		// Release slave again
...
void MFRC522::PCD_Init() {
	if (digitalRead(RESET_PIN) == LOW) { //The MFRC522 chip is in power down mode.
		digitalWrite(RESET_PIN, HIGH);	// Exit power down mode. This triggers a hard reset.
